...
from decluttering.data.tracer.analytic_tracer.utils.utils import *
...
# @profile
def trace(image, start_point_1, start_point_2 = None, stop_when_crossing=False, resume_from_edge=False, timeout=30,
          bboxes=[], viz=False, exact_path_len=None, viz_iter=None, filter_bad=False, x_min=None, x_max=None,
          y_min=None, y_max=None, endpoints=[], keepout_mask=None):
    # viz = False
    image = clean_input_color_image(image.copy(), start_point_1)
    image = cv2.erode(image, np.ones((2, 2)))
    image = np.where(image < 120, 0, 255)
    if keepout_mask is not None:
        image = image * ~keepout_mask
        # plt.imshow(image * ~keepout_mask)
        # plt.show()
    ...
    while len(active_paths) > 0:
        ...
        # print(endpoints)
        # print(active_paths.shape)
        # print(active_paths[0][0][-1][None, :].shape)
    ...
    if viz and len(finished_paths) > 0:
        ...
        plt.show()
        fig.savefig("/home/justinyu/multicable-decluttering/misc_images/debug.png")
